...
namespace McTools.Xrm.Connection
{
    ...
    public class ConnectionManager
    {
        ...
        private object Connect(List<object> parameters, bool retry = false)
        {
            ...
            try
            {
                ...
                _passwordLength = connection.ClientCredentials.UserName.Password.Length;
            ...
            {
                // try again in case of race condition leading to encrypted password
                object returnValue = null;

                if (!retry && !string.IsNullOrEmpty(detail.UserPassword))
                {
                    if (_passwordLength > detail.UserPassword.Length){
                        returnValue = Connect(parameters, true);
                    }
                    else
	                {
                        returnValue = error;
        ...
        public virtual void TestConnection(IOrganizationService service)
        {
            try
            {
